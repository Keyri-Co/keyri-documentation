...
- **Risk Analysis**: Keyri's system ingests data and associates it with events [link to events page], such as registration, login, access, or transaction verification. Data from each event is then analyzed and run through both pre-configured and customized risk signals [link to risk signals page], including device, account, and IP-based signals, to determine the relative risk of each event.
