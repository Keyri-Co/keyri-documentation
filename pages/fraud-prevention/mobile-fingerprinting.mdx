...
- `sendEvent(publicUserId: String?, eventType: String?, success: Bool?)`: Send Keyri a fingerprint of the device
...
void registerUser(String username, String password) {
    ...
    keyri.listAssociationKey(new KeyriCallback<>() {
        @Override
...
fun createUser(username: String): String? {
    ...
    return if (keyri.listAssociationKey().size < MAX_ACCOUNT_LIMIT) {
        ...
        // Optionally sign custom data with the key at this stage
        // remember, the key can always be retrieved with getAssociationKey
        keyri.sendEvent(publicUserId: username, eventType: EventType.Login, eventResult: FingerprintLogResult.Success)
            ...
            if (result.size() < MAX_ACCOUNT_LIMIT) {
                keyri.generateAssociationKey(username, new KeyriCallback<>() {
                    ...
                    public void onComplete(String result) {
                        ...
                        // Optionally sign custom data with the key at this stage
                        // remember, the key can always be retrieved with getAssociationKey
                        keyri.sendEvent(username, EventType.Login, FingerprintLogResult.Success, new KeyriCallback<FingerprintEventResponse>() {
                            @Override
