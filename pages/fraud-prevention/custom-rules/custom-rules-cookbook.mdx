...
Your compliance rules say that a user can't have more than 5 transactions over $1,000 in a 72 hour
time period. Here's how you can do that:

```json copy
{
  // This is what we're calling the rule
  "rule": "Transaction Anomaly Detected",
  // This is what happens if our "conditions" are true
  "outcome": "deny",
  // This is the rule's unique identifier
  "signal": "transaction_anomaly_detected",
  // This means the rule is to be used if it is TRUE
  "enabled": true,
  // This means the rule overrides anything with a strength < 2 if
  ...
  "conditions": {
      ...
      // Congratulations for sticking that out! You deserve a coffee
      // break after all that!
      //
      // We're out of the "$transform" function and can now do logic
      // against the values we added to the 
      // object ("actualValue", "testValue")
      //
      // EVALUATE THE WHOLE THING AS TRUE with an explicit AND
      "$and": [
...
- `rule`: The name of the rule, here it's "Transaction Anomaly Detected" (name it
  whatever you want).
- `outcome`: Set to "deny", so if the rule evaluates to true, the event is
  denied.
- `strength`: Doesn't have to be set. We're letting it default to 2.
- `conditions`: Defines the criteria for the rule. 
  - `$transform`: This function is an array of things that you want _add_ to the object, or things you want to update. In this case, we're writing the number `4` directly to the object so we can use it in logic later. We're also summing up all transactions with the `reduce` function.