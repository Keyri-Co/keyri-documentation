...
func sendFingerprintToBackend(fingerprint: [String: Any]) { guard let url =
URL(string: BASE_URL + "api/fingerprint") else { print("Can't parse request URL:
\(urlString)")
...
function sendFingerprintToBackend(body: string): Promise<Response> { return
fetch(BASE_URL + 'api/fingerprint', { method: 'POST', headers: { Accept:
'application/json', 'Content-Type': 'application/json', }, body: body, }); }
