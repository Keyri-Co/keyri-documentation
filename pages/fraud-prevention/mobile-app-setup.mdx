...
- `generateAssociationKey(publicUserId: String, completion: (Result<P256.Signing.PublicKey, Error>) -> ())`:
  Create a persistent record of a user on the device
- `getAssociationKey(publicUserId: String, completion: (Result<P256.Signing.PublicKey?, Error>) -> ())`:
  Retrieve key for a given user, if the user has been created
- `listAssociationKeys(completion: (Result<[String:String]?, Error>) -> ())`:
  List the keys that have been created on the device
- `listUniqueAccounts(completion: (Result<[String:String]?, Error>) -> ())`:
  List the unique accounts associated with the device
- `generateUserSignature(publicUserId: String, data: Data, completion: (Result<P256.Signing.ECDSASignature, Error>) -> ())`:
  Sign a piece of data with the user's key

#### Full Fraud Analytics and Prevention

- `sendEvent(publicUserId: String, eventType: EventType, success: Bool, completion: (Result<FingerprintResponse, Error>) -> ())`:
  Send Keyri a fingerprint of the device directly from the app and have the
  results returned to the mobile app
- For Andrew: please add information about the new fingerprinting method you
  made

### Example: Protecting Registration/Signup

For Andrew: please update these examples and make them more relevant to what we
do now, including an example of the new fingerprinting method.
...
altogether, etc. All you're getting when running `listUniqueAccounts()` is a
useful, reliable array of accounts associated with that device, and you can
parse and act upon it however you'd like.
...
fun registerUser(username: String, password: String) {
    ...
    val keys = keyri.listAssociationKeys()
...
void registerUser(String username, String password) {
    ...
    keyri.listAssociationKeys(new KeyriCallback<>() {
        @Override
        public void onComplete(Map<String, String> result) {
            if (result.isEmpty()) {
                keyri.generateAssociationKey(username, new KeyriCallback<>() {
                    @Override
...
function registerUser(username: string, password: string) {
    ...
    const keys = Keyri.listAssociationKeys();
...
fun createUser(username: String): String? {
    ...
    return if (keyri.listAssociationKeys().size < MAX_ACCOUNT_LIMIT) {
...
void createUser(String username) {
    ...
    keyri.listAssociationKeys(new KeyriCallback<>() {
        @Override
        public void onComplete(Map<String, String> result) {
            if (result.size() < MAX_ACCOUNT_LIMIT) {
                keyri.generateAssociationKey(username, new KeyriCallback<>() {
                    @Override
                    public void onComplete(String result) {
                        String key = result;
                        ...
                        // Optionally sign custom data with the key at this stage
                        // remember, the key can always be retrieved with getAssociationKey
                        keyri.sendEvent(username, EventType.Login, true, new KeyriCallback<FingerprintEventResponse>() {
                            @Override
                            public void onComplete(FingerprintEventResponse result) {
                                // Process result
                            }
