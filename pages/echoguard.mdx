...
Keyri's EchoGuard is a simple, secure, and inexpensive alternative to SMS OTPs for verifying ownership of a user's phone number. Rather than sending a code to the user's phone number, EchoGuard guides your user to send a code to us in just two taps. Once we receive the code, we inform you via webhook that the phone number has been verified.

## Phone Number Acquisition

With EchoGuard, you can simultaneously acquire and validate a user's phone number without them having to type it in. This is done by having the user send a text message to a number that we provide. The user's phone number is then sent to you via webhook.

### Set up your Callback URL

First, create an endpoint on your backend that accepts a POST request. We will send a webhook to this endpoint when the user sends a text message to the number we provide. The request body will contain the user's phone number and any metadata you provided when you made the API call, as follows:
...
curl --request POST \
     --url https://echoguard.keyri.com/acquire-phone-number \
...
iOS features `MFMessageComposeViewController`, which is a native view controller that allows the user to compose and send text messages in a modal without leaving your application. You can pre-compose the message and recipient for the user so that they only have to tap "Send". See [Apple's documentation](https://developer.apple.com/documentation/messageui/mfmessagecomposeviewcontroller) for full details.
...
```shell filename="Request" copy
curl --request POST \
     --url https://echoguard.keyri.com/make-confirmation \
     --header 'Content-Type: application/json' \
     --header 'x-api-key: [YOUR-API-KEY]' \
     --data-raw '
  {
...
sequenceDiagram
    ...
    participant Server
    participant EchoGuard
    User -->> Client App: Initiate
    Client App ->> Server: 
    Server ->> Server: Make record of validation request
    Server ->> EchoGuard: 
    EchoGuard ->> Server: SMS body and recipient
    Server ->> Server: Update request record
    Server ->> Client App: 
    Client App ->> User: SMS button or QR code
    User -->> EchoGuard: Send SMS
    EchoGuard ->> EchoGuard: Validate message
    EchoGuard ->> Server: {phoneNumber, valid}
    Server ->> Server: Update request record
    Server ->> Client App: Notify
```