...
QR login allows your users to log into your web app just by scanning a QR code and confirming the login in your mobile app. This is a great way to make your app more secure and convenient for your users.
...
1. [Web frontend](/authentication/qr-authentication/keyri-qr-widget) (wherever the QR code is shown, for example, desktop or smart TV login page)
2. [Mobile app](/mobile-sdks) (use our mobile SDK in your app to send payloads to the Widget)
3. Backend (optional: only required if going through the [server-side authentication](/authentication/qr-authentication/server-side-authentication) strategy)

If you're ready to get started, you can [sign up for a free account](https://app.keyri.com/sign-up).

Then proceed to install the [Keyri QR Widget](/authentication/qr-authentication/keyri-qr-widget) on your web frontend and the [Keyri SDK](/mobile-sdks) in your mobile app.
...
For a more visual representation of this flow and some more details into its inner workings, see the diagrams on the [client session extension](/authentication/qr-authentication/client-session-extension) and [server-side authentication](/authentication/qr-authentication/server-side-authentication) documentation pages, each of which represent two distinct ways to implement QR login with Keyri. The vast majority of the steps you see here are automated by the Keyri Widget and mobile SDKs.
