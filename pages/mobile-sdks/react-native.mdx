            ...
            //render content based on session object, e.g. location, country code
             ...
             <View style={styles.customPopupButtonView}>
                ...
                <TouchableOpacity
                    ...
                    onPress={() => { Keyri.confirmSession('custom_payload'); }>
...
- `initialize: (options: InitializeKeyriOptions)` - call it before all Keyri operations

- `easyKeyriAuth: (publicUserId: string, payload: string)` - handle process flow with passed scanned url and showing default confirmation screen. Easiest way to process session from deeplink
...
- `initializeDefaultConfirmationScreen: (sessionId: string, payload: string) ` - to show Confirmation with default UI. Returns Boolean result. Also, you can implement your custom Confirmation Screen, just inherit from BaseConfirmationDialog.kt

- `confirmSession: (payload: string, trustNewBrowser?: boolean)` - call this function if user confirmed the dialog, trustNewBrowser is false by default. Returns Boolean authentication result

- `denySession: (payload: string)` - call if the user denied the dialog. Returns Boolean authentication result
...
- `generateUserSignature: (publicUserId?: string, data?: string)` - returns an ECDSA signature of the timestamp and optional data with the publicUserId's privateKey (or, if not provided, anonymous privateKey), data can be anything

- `listAssociationKeys: ()` - returns a list of names (publicUserIds) of "association keys" (public keys)
...
- `sendEvent: (data: SendEventOptions)` - sends fingerprint event with specified event type and result. Returns the event risk response object from the Keyri API
