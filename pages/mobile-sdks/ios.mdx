...
- `func easyKeyriAuth(userId: String, payload: String): Result<Bool, Error>`- call to have Keyri drive you through the entire process - we display the scanner, scan the QR code, handle user confirmation and fire off the result to the browser - all with one line of code in your app 😀
