...
- `func init(appKey: String, publicAPIKey: String?, serviceEncryptionKey: String?)`- Initialize the Keyri object with the keys retrieved from the dashboard. If you're using sendEvent() to take advantage of our fraud dashboard, the publicAPIKey and serviceEncryptionKey are required

- `func sendEvent(publicUserId: String, eventType: EventType, success: Bool): FingerprintResponse` - Sends an event to our dashboard containing a device snapshot to our dashboard. The response you recieve will be sent, as is, to your own backend, where you can utilize our scripts to help you decrypt. See the code sample below and https://docs.keyri.com/fraud-prevention for more

- `func initializeQrSession(sessionId: String): Result<Session, Error>` - call after obtaining the sessionId from QR-code or deep link. Returns Session object with Risk attributes (needed to show confirmation screen) or Exception

- `func initializeDefaultConfirmationScreen(session: Session, payload: String): Boolean` - to show Confirmation with default UI. Alternatively, you can implement a custom Confirmation Screen. The Default screen is built using SwiftUI, however the session object is designed to work seamlessly with UIKit as well should you prefer that route
...
- `func processLink(url: URL, publicUserId: String, appKey: String, payload: String): Result<Bool, Error> ` - Handles entire QR flow just like easyKeyriAuth below, but from after the QR code is scanned. Useful if you'd like to use a custom QR scanner rather than the one we provide. Simply have the user scan the code, and pass us the URL. Everything else is taken care of


```
keyri.sendEvent(publicUserId: username, eventType: eventType, success: true) { fingerprintResponse in
	let dict = [
	"username": username,
	"password": password,
	"publicKey": "C1uC8r7C53NcF/qaUPFDSEoAOh2sbvOLjAetcEKDs1eC40Vx14NMikNJu4aTGD4C9ijL2mvQYekEGxOy2Oj1lA==", // Obtained from dashboard
	"encryptedSignupEventString": self.stringifyFingerprintResponse(fingerprintResponse)! // see below, force unwrapped for brevity
	] as [String : String]
	
	let url = URL(string: "https://fraud-demo.keyri.com/api/signup")!
	// cut short for brevity, but at this point simply send a POST to the URL using the dict as the http body, and you're done!
...
- `func generateUserSignature(publicUserId: String, data: Data): P256.Signing.ECDSASignature` - returns an ECDSA signature of the timestamp and optional customSignedData with the publicUserId's privateKey (or, if not provided, anonymous privateKey), data can be anything

- `func easyKeyriAuth(publicUserId: String, payload: String): Result<Bool, Error>`- call to have Keyri drive you through the entire process - we display the scanner, scan the QR code, handle user confirmation and fire off the result to the browser - all with one line of code in your app 😀
...
- `func removeAssociationKey(publicUserId: String)` - Removes key associated with the user id provided. Throws error if the given user id does not have an association key saved

- `func listAssociationKeys(): [String: String]` - Lists all saved associationKeys as a map, with the key being the userId and value being public key

- `func listUniqueAccounts(): [String]` - Lists all saved userIds with a valid associationKey saved
