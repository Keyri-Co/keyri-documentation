---
description: Keyri QR authentication provides Auth0 IAM customers with a secure, easy-to-use, and convenient authentication system for their end-users.
---
...
Auth0
    ...
    .start { result in
        ...
        case .success(let credentials):
            ...
            let keyri = KeyriInterface(appKey: "BQzgy1MhPcLVCACpLX7wn25BIFv1nd2q", publicApiKey: publicApiKey, serviceEncryptionKey: serviceEncryptionKey)
            let payload = "{\"refreshToken\":\"\(credentials.refreshToken ?? "")\"}"
            print(payload)

            //3
            keyri.easyKeyriAuth(payload: payload, publicUserId: username) {
                //{"refreshToken": "[token]"}
                reponse in
                print(reponse)
            }
