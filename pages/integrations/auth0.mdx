---
description: How to enable passwordless QR login with Auth0
---
...
![Create Application](https://archbee-image-uploads.s3.amazonaws.com/FQ4YmCkDokMJylbTAsoOR/BQQtgoZkcqJuDPN5NOGRo_screen-shot-2022-10-31-at-174736.png)

On Auth0's console, create a new application and select Native.

### Configure URIs for Mobile App

![](https://archbee-image-uploads.s3.amazonaws.com/FQ4YmCkDokMJylbTAsoOR/9M6USbkHsBdZL2glGrBCM_screen-shot-2022-10-31-at-174809.png)
...
Auth0
    ...
    .start { result in
        ...
        case .success(let credentials):
            ...
            //3
            keyri.easyKeyriAuth(publicUserId: "", appKey: "BQzgy1MhPcLVCACpLX7wn25BIFv1nd2q", payload: payload) {
                //{"refreshToken": "[token]"}
                reponse in
                print(reponse)
            }
    ...
    val profileCallback = object : Callback<UserProfile, AuthenticationException> {
        ...
        override fun onSuccess(result: UserProfile) {
            ...
            easyKeyriAuth(
                ...
                appKey = "BQzgy1MhPcLVCACpLX7wn25BIFv1nd2q",
                payload = payload,
                publicUserId = email
            )
        }
