...
- `generateUserSignature(username: String?, data: Data) -> Signature`: Sign a piece of data with the user's key
...
func registerUser(username: String) throws {
    let keyri = Keyri() // Initialize SDK
    ...
    if let list = keyri.listAssociationKey() {
        if let existingUsername = list.keys.first {
            ...
            keyri.sendEvent(username, "signup", false)
            // Send failed signup fingerprint event
        }
    } else {
       ...
       keyri.sendEvent(username, "signup", true)
       // Send success signup fingerprint event
    }
}
````
</Tab>
...
suspend fun registerUser(username: String, password: String) {
    ...
    keyri.sendEvent(username, EventType.Signup, FingerprintLogResult.Incomplete)
    // Send fingerprint event to signalize pending signup intent

    if (keys.isNotEmpty()) {
        // Alert user that there is an existing user, and encourage them to sign in here

        keyri.sendEvent(username, EventType.Signup, FingerprintLogResult.Fail)
        // Send failed signup fingerprint event
    } else {
        ...
        keyri.sendEvent(username, EventType.Signup, FingerprintLogResult.Success)
       // Send success signup fingerprint event
    }
}
````
...
async function registerUser(username: string, password: string) {
    ...
    keyri.sendEvent({ publicUserId: username, eventType: EventType.Signup, eventResult: FingerprintLogResult.Incomplete })
    // Send fingerprint event to signalize pending signup intent

    if (keys?.length > 0) {
        // Alert user that there is an existing user, and encourage them to sign in here

        keyri.sendEvent({ publicUserId: username, eventType: EventType.Signup, eventResult: FingerprintLogResult.Fail })
        // Send failed signup fingerprint event
    } else {
        ...
        keyri.sendEvent({ publicUserId: username, eventType: EventType.Signup, eventResult: FingerprintLogResult.Success })
        // Send success signup fingerprint event
    }
}
````
