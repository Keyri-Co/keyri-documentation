...
# Overview
Keyri offers best in class mobile fingerprinting and device integrity verification, and it’s all accessible in as little as one line of code.

The SDK will:
Generate a unique fingerprint which persists even when the app is deleted
This is something many developers struggle with. It’s quite easy to keep a fingerprint UNTIL the app is deleted, but persistence is tricky. This is exacerbated by the ease with which bad actors can clear out records in the Keychain or Android Secure Storage.
...
iOS: https://docs.keyri.com/mobile-sdks/ios
	Step 1: add “keyri-pod” to podfile
	Step 2: run “pod install”
	Step 3 “import keyri_pod” to whichever file you are triggering Fingerprinting from
...
* generateFingerprint(eventType: String?, customData: JSON?): Send Keyri a fingerprint of the device
...
func createUser(username: String) -> P256.Signing.PublicKey? {
	...
	// Optionally sign custom data with the key at this stage
		...
		Keyri().generateFinerprint(eventType: .login, custom: [Custom data here])
		Return key
	} else {
...
fun createUser(username: String): String? {
    ...
    return if (keyri.listAssociationKey().size < MAX_ACCOUNT_LIMIT) {
        ...
        // Optionally sign custom data with the key at this stage
        // remember, the key can always be retrieved with getAssociationKey
        keyri.generateFinerprint(eventType: EventType.Login, custom: "[Custom data here]")
