...
* _qr_event_session_validate_ - Decrypted data from the __mobile app__ from an approved a login request.

* _qr_event_socket_error_ - This is when there's an error coming from the API. 

* _qr_event_risk_data_ - Risk Data about the connection coming from the API that was also signed by the API.

* _qr_event_appless_login_ - If you're using mobile QR login, this is what will be emmitted when the user is logged in. __Alert the user or reload the page.__

* _qr_event_appless_registration_ - When the user registers their phone via QR for appless. __Alert the user their request was successful.__

* _qr_event_mobile_connect_ - this is when a mobile device connects to your QR Code. The QR code becomes blurry.
